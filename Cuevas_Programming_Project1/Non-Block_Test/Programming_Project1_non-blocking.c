...
#include <sys/time.h>
#include <stdlib.h>
/******************* MPI Non-Blockig Send/Receive program *********************/
int main(int argc,char *argv[])
{
    ...
    MPI_Comm_rank(MPI_COMM_WORLD,&rank); //communicates rank of processor
    MPI_Comm_size(MPI_COMM_WORLD,&p); // communicates the total number of processors in the progr>

    assert(p>=2);

    int loops=10; // 10 sends 10 reveives.
    int i = 0;
    int j = 0;
    ...
    if(rank==1)
    {
        ...
        for (j= 0; j <= 20; ++j)
        {
            ...
            for(i = 0; i < loops; ++i)
            {
                ...
                (2) number of elements in send buffer // length/size of message (nonneg int),
...
//Proccess B--------------------------------------------------------------------
    else if (rank==0)
    {
        ...
        MPI_Irecv(&test_y,1,MPI_CHAR,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,&request1);
        ...
        for (j= 0; j <= 20; ++j)
        {
            ...
            for(i=0; i < loops; ++i)
            {
                MPI_Irecv(y,size,MPI_CHAR,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,&request1);
                MPI_Wait(&request1,&status);
            }
            //----------------------End timer--------------------------------//
            gettimeofday(&t2,NULL);
